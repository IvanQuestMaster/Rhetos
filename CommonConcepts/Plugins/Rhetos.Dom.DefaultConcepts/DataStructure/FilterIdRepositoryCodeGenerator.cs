...
namespace Rhetos.Dom.DefaultConcepts
{
    ...
    public class FilterIdRepositoryCodeGenerator : IConceptCodeGenerator
    {
        ...
        public IEnumerable<Guid> CreateQueryableFilterIds(IEnumerable<Guid> ids)
        {
            ...
            string sqlInsertIdFormat = ""INSERT INTO Common.FilterId (Handle, Value) VALUES ('"" + _sqlUtility.GuidToString(handle) + ""', '{0}');"";
            ...
            for (int start = 0; start < ids.Count(); start += chunkSize)
            {
                string sqlInsertIds = string.Join(""\r\n"", ids.Skip(start).Take(chunkSize)
                        .Select(id => string.Format(sqlInsertIdFormat, _sqlUtility.GuidToString(id))));
            ...
            string deleteFilterIds = ""DELETE FROM Common.FilterId WHERE Handle = "" + _sqlUtility.QuoteGuid(handle);
