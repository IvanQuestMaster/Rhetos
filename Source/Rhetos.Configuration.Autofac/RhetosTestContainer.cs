                ...
                if (_iocContainer == null)
                {
                    lock (_containerInitializationLock)
                        if (_iocContainer == null)
                        {
                            ...
                            var runtimeContext = Host.Find(Path.Combine(rhetosAppRootPath, "bin"));
                            var configurationProvider = runtimeContext.BuildConfiguration(new ConsoleLogProvider(),
                                Path.Combine(rhetosAppRootPath, "bin"), null);

                            AppDomain.CurrentDomain.AssemblyResolve += SearchForAssembly;

                            var sw = Stopwatch.StartNew();
                            _iocContainer = runtimeContext.BuildContainer(new ConsoleLogProvider(), configurationProvider, (builder) => {
