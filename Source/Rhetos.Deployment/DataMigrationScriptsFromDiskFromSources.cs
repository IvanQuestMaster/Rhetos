...
{
    public class DataMigrationScriptsFromDiskFromSources : IDataMigrationScriptsProvider
    {
        const string DataMigrationSubfolder = "DataMigration";
        const string DataMigrationSubfolderPrefix = DataMigrationSubfolder + @"\";

        protected readonly RhetosOptions _options;
        private readonly FilesUtility _filesUtility;

        public DataMigrationScriptsFromDiskFromSources(RhetosOptions options, FilesUtility filesUtility)
        {
            _options = options;
        ...
        public List<DataMigrationScript> Load()
        {
            ...
            foreach (var package in _options.Sources)
            {
                var files = Directory.GetFiles(Path.Combine(package, DataMigrationSubfolder), "*", SearchOption.AllDirectories);

                const string expectedExtension = ".sql";
                var badFile = files.FirstOrDefault(file => !string.Equals(Path.GetExtension(file), expectedExtension, StringComparison.OrdinalIgnoreCase));
                if (badFile != null)
                    throw new FrameworkException("Data migration script '" + badFile + "' does not have expected extension '" + expectedExtension + "'.");

                var packageScripts =
                    (from file in files
                     let scriptContent = _filesUtility.ReadAllText(file)
                     select new DataMigrationScript
                     {
                         ...
                         Path = Directory.GetParent(package).Name,
